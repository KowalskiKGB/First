...
export function verifyDevPassword(password, encoded) {
  ...
  const actual = crypto.scryptSync(String(password), match[1], 32);
...
export function resolveDevCredential(env = process.env) {
  ...
  if (!SCRYPT_PATTERN.test(passwordHash)) throw new Error('DEV_PANEL_PASSWORD_HASH must be a scrypt hash');
