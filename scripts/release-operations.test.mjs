...
test('release backup and restore fail closed around the single JSON writer', () => {
  ...
  assert.doesNotMatch(restore, /realpath --/)
  assert.match(restore, /mktemp -d[^\n]+first-restore-private/)
  assert.match(restore, /chmod 700[^\n]+private/)
  assert.match(restore, /chmod 600[^\n]+snapshot/)
  assert.match(restore, /exec 9<[^\n]+snapshot/)
  assert.match(restore, /rm -f[^\n]+snapshot/)
  assert.match(restore, /\/dev\/fd\/9/)
  assert.match(restore, /tar -tvzf/)
  assert.match(restore, /tar -tzf "\$snapshot_fd_path"/)
  assert.match(restore, /< "\$snapshot_fd_path"/)
  assert.match(restore, /invalid_entries=\$\(find[^\n]+\) \|\| exit/)
  assert.match(restore, /hardlinked_files=\$\(find[^\n]+\) \|\| exit/)
  ...
  const validate = restore.indexOf('tar -tzf "$snapshot_fd_path"')
