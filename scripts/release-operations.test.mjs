...
test('release backup and restore fail closed around the single JSON writer', () => {
  ...
  const stop = restore.indexOf('docker compose stop api', validate)
  ...
  assert.match(runbook, /symbolic links and hard links|symlinks and hardlinks/i)
  assert.match(runbook, /connect-timeout|bounded health/i)
  assert.match(runbook, /writer stop cannot be confirmed|stop.*writer.*confirmed/i)
})
