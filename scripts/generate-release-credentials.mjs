...
function parseArguments(argv) {
  ...
  const allowed = new Set(['--url', '--credentials-out', '--handoff-out', '--scope'])
  if ([...values.keys()].some(name => !allowed.has(name))) fail('unknown option')
  for (const required of ['--url', '--credentials-out', '--handoff-out']) if (!values.get(required)) fail(`missing required option: ${required}`)
...
function devadminUrl(origin) {
  return `${origin}/devadmin`
}
...
function credentialDocument({ url, username, password, generatedAt, copyable = false }) {
  const mark = value => copyable ? value : `\`${value}\``
  const usernameLabel = 'Usuário'
  return [
    '# Credenciais de teste — First',
    '',
    `- URL: ${url}`,
    `- ${usernameLabel}: ${mark(username)}`,
    `- Senha: ${mark(password)}`,
    `- Gerado em: ${mark(generatedAt)}`,
    '',
...
export function generateReleaseCredentials(argv = process.argv.slice(2), overrides = {}) {
  ...
  if (!['all', 'dev'].includes(scope)) fail('scope must be all or dev')
  ...
  const handoff = {
    ...
    ...(scope === 'dev' ? {} : { AI_CONFIG_MASTER_KEY: crypto.randomBytes(32).toString('hex') }),
  }
  ...
  try {
    publishPrivateOutput(
      credentialsPath,
      credentialDocument({ url: devadminUrl(url), username, password, generatedAt, copyable: scope === 'dev' }),
      runtime,
      owned,
    )
    publishPrivateOutput(
