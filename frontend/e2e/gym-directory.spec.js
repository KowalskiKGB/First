...
const gyms = [
  {
    ...
    latitude: 0.031, longitude: -51.067, distanceKm: 0.7, averageRating: 4.8, reviewCount: 27,
  ...
  },
  {
    ...
    distanceKm: 1.6, averageRating: 4.5, reviewCount: 8, tags: ['Perto de você'], openingHours: [], exerciseIds: ['0007'],
  },
  {
...
function fixtures({ authenticated = false } = {}) {
  ...
  return {
    ...
    async handle(route) {
      ...
      if (method !== 'GET') writes.push({ method, pathname: url.pathname, body })
...
test('student favorites, reviews, contributes, selects and creates on desktop', async ({ page }, testInfo) => {
  const api = fixtures({ authenticated: true })
  const errors = watchBrowser(page)
  ...
  expect(kinds).toEqual(['correction', 'equipment', 'closure', 'gym'])
