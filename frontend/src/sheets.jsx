...
function AddToRoutine({ ex, close }) {
  ...
  return <>
    <h3 className="capitalize exercise-name">{t('Add “{0}”', exerciseName(ex))}</h3>
...
function CustomExForm({ existing, prefill, onDone, close }) {
  ...
  const save = () => {
    ...
    const dup = allExercises(S()).find(e =>
      e.id !== (existing || {}).id && [e.n, exerciseName(e)].some(label => searchKey(label) === nameKey))
    if (dup) { toast(t('“{0}” already exists', exerciseName(dup))); return }
...
function ExercisePicker({ onPick, close }) {
  ...
  return <>
    ...
    <div className="search"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7" /><path d="m21 21-4.3-4.3" /></svg>
      <input className="input" name="exercise-picker-search" aria-label={t('Search {0} exercises…', all.length)} autoComplete="off" placeholder={t('Search {0} exercises…', all.length)} value={q} onChange={e => { setQ(e.target.value); setShown(50) }} /></div>
    ...
    <div className="list">
      ...
      {f.slice(0, shown).map(e => <button type="button" key={e.id} className="item" onClick={() => onPick(e)}>
        <Thumb ex={e} /><div className="grow"><div className="tt capitalize exercise-name">{exerciseName(e)}</div><div className="ss capitalize">{t(e.tg || e.bp)} · {t(e.eq)}</div></div>
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = APP_SLUG + '-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own openGym — routines only, none of your workouts or weigh-ins.').replace('openGym', APP_NAME)}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function DayOverride({ iso, close }) {
  ...
  return <>
    ...
    <div className="list">
      ...
      <button type="button" className="item" onClick={() => set('rest')}><span className="lrow-i" style={{ background: 'var(--surface-3)' }}><Icon name="moon" /></span><div className="grow"><div className="tt">{t('Prefer rest for this day')}</div><div className="ss">{t('Managed sessions remain available.')}</div></div>{effId === null && <Icon name="check" className="accent" />}</button>
...
function DayAssign({ day, close }) {
  ...
  return <>
    ...
    <div className="list">
      ...
      {st.routines.filter(r => !r._personalProgramId && r._aiGenerated !== true).map(r => <button type="button" key={r.id} className="item" onClick={() => set(r.id)}>
        <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
        <div className="grow"><div className="tt">{r.name}</div><div className="ss">{exCount(r.ex.length)}</div></div>
  ...
  return <>
    ...
    {w.entries.map((e, i) => {
      ...
      return <div key={i} className="row" style={{ marginBottom: 12, alignItems: 'flex-start' }}>
        {ex && <Thumb ex={ex} />}
        <div className="grow"><div className="tt capitalize exercise-name" style={{ fontWeight: 600 }}>{ex ? exerciseName(ex) : (e.n || e.id)} {w.prs && w.prs.includes(e.id) && <span className="pr"><Icon name="trophy" />PR</span>}</div>
...
function TopWeight({ entryIdx, close }) {
  ...
  return <>
    <h3 className="capitalize exercise-name row" style={{ gap: 8 }}><Icon name="checkCircle" style={{ color: 'var(--acc)' }} />{t('{0} done', exerciseName(ex))}</h3>
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    {(prs.length > 0 || e1prs.length > 0) && <div style={{ textAlign: 'left', marginBottom: 12 }}>
      {prs.map(id => <div key={id} className="small accent capitalize exercise-name row" style={{ gap: 5 }}><Icon name="trophy" style={{ fontSize: 13 }} />{t('New PR:')} {exerciseName(EXIDX[id]) || id}</div>)}
      {e1prs.map(p => <div key={p.id} className="small accent capitalize exercise-name row" style={{ gap: 5 }}><Icon name="chartLine" style={{ fontSize: 13 }} />{t('Best estimated 1RM:')} {exerciseName(EXIDX[p.id]) || p.id} · {fmtNum(p.est)} {st.unit}</div>)}
    </div>}
