...
import { AI_EQUIPMENT, AI_EXPERIENCE, AI_TARGET_AREAS, equipmentLabel } from '../../lib/ai-plan.js'
...
function ToggleGrid({ label, values, options, onChange, className = '' }) {
  ...
  return (
    <fieldset className={`ai-choice-group ${className}`}><legend>{label}</legend><div className="ai-toggle-grid">
      {options.map(([value, optionLabel]) => <button type="button" key={value} aria-pressed={values.includes(value)} onClick={() => toggle(value)}>{optionLabel}</button>)}
    </div></fieldset>
  )
}
    ...
    <PersonalMutation path="/api/personal/training-profile" method="PUT" success="Training profile updated">
      {({ submit, busy }) => <form className="personal-form ai-personal-form" aria-label={t('Edit AI training profile')} onSubmit={event => { event.preventDefault(); submit({ clientId: client.id, ...draft }) }}>
        <div className="personal-form-grid">
          ...
          <label className="form-field"><span>{t('Primary goal')}</span><TextField name="personal-ai-goal" value={draft.goal} onChange={event => setDraft({ ...draft, goal: event.target.value })} required /></label>
          <label className="form-field"><span>{t('Experience')}</span><select className="field" value={draft.experience} onChange={event => setDraft({ ...draft, experience: event.target.value })}>{AI_EXPERIENCE.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        ...
        <label className="form-field"><span>{t('Limitations')}</span><TextArea name="personal-ai-limitations" value={draft.limitations} onChange={event => setDraft({ ...draft, limitations: event.target.value })} /></label>
        <div className="consent-checks">
          <label><input type="checkbox" checked={draft.consent} onChange={event => setDraft({ ...draft, consent: event.target.checked })} />{t('Student consent confirmed')}</label>
          {draft.ageBand !== 'adult' ? <label><input type="checkbox" checked={draft.guardianConsent === true} onChange={event => setDraft({ ...draft, guardianConsent: event.target.checked })} />{t('Guardian confirmation')}</label> : null}
          <label><input type="checkbox" checked={draft.acuteRisk} onChange={event => setDraft({ ...draft, acuteRisk: event.target.checked })} />{t('Acute risk reported')}</label>
          <label><input type="checkbox" checked={draft.medicalRestriction} onChange={event => setDraft({ ...draft, medicalRestriction: event.target.checked })} />{t('Medical restriction reported')}</label>
        </div>
  ...
  return (
    <PersonalMutation path="/api/personal/gym" method="PUT" success="Gym updated">
      {({ submit, busy }) => <form className="personal-form ai-personal-form" aria-label={t('Edit student gym')} onSubmit={event => { event.preventDefault(); submit({ clientId: client.id, ...draft }) }}>
        <label className="form-field"><span>{t('Gym')}</span><TextField name="personal-ai-gym" value={draft.name} onChange={event => setDraft({ ...draft, name: event.target.value })} required /></label>
        ...
        {draft.specificMachines.length ? <div className="specific-machine-list"><span className="personal-eyebrow">{t('Specific machines')}</span>{draft.specificMachines.map((machine, index) => <div key={`${machine.name}-${index}`}><strong>{machine.name}</strong><span>{machine.category || t('No category')} · {t('{0} linked exercises', machine.exerciseIds.length)}</span></div>)}</div> : null}
  ...
  return (
    <div className="ai-plan-dossier">
      ...
      <dl className="detail-list">
        <div><dt>{t('Provider')}</dt><dd>{plan.provider} · {plan.model}</dd></div>
        <div><dt>{t('Applied')}</dt><dd>{plan.appliedAt ? fmtDate(plan.appliedAt.slice(0, 10), true) : '—'}</dd></div>
        <div><dt>{t('Context')}</dt><dd><code>{String(plan.contextHash || '').slice(0, 16) || '—'}</code></dd></div>
      </dl>
  ...
  return (
    <div className="personal-ai-layout">
      ...
      {grants.aiPlanRead ? <section className="personal-panel"><div className="panel-heading"><h2>{t('Applied AI plan')}</h2></div><PlanSummary plan={client.aiPlan} client={client} />
        {measurements.length ? <div className="measurement-used"><span className="personal-eyebrow">{t('Latest measurements available')}</span><div>{measurements.slice(0, 6).map(item => <span key={item.id}>{t(item.kind)} <b>{item.value.toLocaleString('pt-BR')} {item.unit}</b></span>)}</div></div> : null}
      </section> : null}
