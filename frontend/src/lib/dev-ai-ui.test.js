...
describe('Dev AI panel helpers', () => {
  ...
  it('maps Dev failures to safe copy without echoing provider or credential details', () => {
    ...
    expect(safeDevError({ status: 502, message: 'AI provider request timeout.' }, 'Operation failed')).toBe('The provider took too long to respond.')
    expect(safeDevError({ status: 502, message: 'Gemini model request failed (429)' }, 'Operation failed')).toBe('The provider request limit was reached. Try again later.')
    expect(safeDevError({ status: 422, message: 'Gemini model request failed (429)' }, 'Operation failed')).toBe('The provider request limit was reached. Try again later.')
    expect(safeDevError({ status: 502, message: 'Gemini model request failed (400)' }, 'Operation failed')).toBe('The selected model rejected the structured test. Try another model.')
    expect(safeDevError({ status: 502, message: 'AI provider request failed (503)' }, 'Operation failed')).toBe('The provider is temporarily unavailable. Try again later.')
    expect(safeDevError({ status: 502, message: 'Gemini model request failed (200)' }, 'Operation failed')).toBe('Operation failed')
