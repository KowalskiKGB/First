...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR First instance — just the
...
export function parsePlan(raw) {
  ...
  if (!data || !data.opengym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an openGym plan file').replace('openGym', APP_NAME))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with openGym').replace('openGym', APP_NAME))} · ${APP_URL.replace(/^https?:\/\//, '')}</footer>
</div></body></html>`
}
