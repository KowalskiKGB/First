...
Revisado até: `a944ee6b7956cd74e41562151961ef4829a6da22`

## Resumo executivo

**Aprovado para o deploy privado de instância única, sem achados críticos ou altos abertos.** O diferencial tem 100 arquivos, 9.559 adições e 998 remoções. A revisão foi classificada como alta criticidade por alterar autenticação Dev, criptografia de chaves, chamadas externas, autorização entre aluno e Personal, aplicação automática de planos e recuperação do volume.
...
- Backup/restore agora param e confirmam o único writer, copiam o arquivo de origem uma única vez para snapshot privado e imutável, rejeitam links/tipos especiais antes da troca, mantêm recovery e só fazem rollback após nova confirmação de parada; o gerador Dev publica arquivos privados com link exclusivo, sem sobrescrever alvos, e separa senha local do handoff efêmero (`scripts/backup-first-data.sh`, `scripts/restore-first-data.sh`, `scripts/generate-release-credentials.mjs`).
...
- Release operacional: 21/21 testes comportamentais/contratuais, deployment 2/2 e sintaxe Bash dos dois scripts validada no Alpine 3.22. Os casos cobrem troca concorrente do arquivo de origem, publicação concorrente das credenciais, API ainda ativa após `stop`, links, FIFO e device.
...
- O histórico mostra a remoção explícita de `initialPassword`, geração automática em `/data`, fallback para qualquer provedor configurado, retenção excessiva de uso e restore destrutivo. Os commits de endurecimento relevantes incluem `bef5b4d`, `7db2f1f`, `c72737d`, `4671d35`, `b33fe54`, `e1dd575`, `60babb2`, `1007c28`, `4232e78`, `c5dafb7`, `0389e3c`, `bf3dae8` e `a944ee6`.
